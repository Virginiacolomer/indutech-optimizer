...
export default function MonteCarlo() {
  ...
  const chartOpts = { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { grid: { display: false }, ticks: { font: { size: 10 }, maxTicksLimit: 8 } }, y: { grid: { color: '#e8e7e4' }, ticks: { font: { size: 10 } }, title: { display: true, text: 'Frecuencia', font: { size: 10 } } } } };

  return (
    <>
      <h2 style={{ fontSize: 15, fontWeight: 600, marginBottom: 4 }}>Simulación Monte Carlo</h2>
      <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 16 }}>La demanda real puede variar ±{variabilidad}%. Esta simulación corre {nSim.toLocaleString()} escenarios aleatorios y muestra la distribución de costos esperados.</p>
