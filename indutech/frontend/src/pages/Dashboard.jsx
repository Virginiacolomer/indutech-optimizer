...
// Solver local para feedback instantáneo (sin llamada a API)
...
export default function Dashboard() {
  ...
  const chartOpts = { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { grid: { color: '#e8e7e4' }, ticks: { font: { size: 11 } } }, y: { grid: { color: '#e8e7e4' }, ticks: { font: { size: 11 } } } } };
  ...
  return (
    <>
      ...
      {saveMsg && <div className={`alert alert-${saveMsg.type}`}><i className={`ti ti-${saveMsg.type === 'success' ? 'check' : 'alert-circle'}`} />{saveMsg.text}</div>}
      ...
      <div className="grid-2">
        <div className="panel">
          <div className="panel-hd"><span className="panel-title"><i className="ti ti-sliders" />Parámetros</span></div>
          <Slider label="Demanda enero (hs)" id="d1" min={10} max={150} value={params.d1} onChange={setParam} />
          <Slider label="Demanda febrero (hs)" id="d2" min={10} max={150} value={params.d2} onChange={setParam} />
          <Slider label="Demanda marzo (hs)" id="d3" min={10} max={150} value={params.d3} onChange={setParam} />
          <div className="divider" />
          <Slider label="Inventario inicial (hs)" id="inv0" min={0} max={100} value={params.inv0} onChange={setParam} />
          <Slider label="Costo contratar ($/hs)" id="ch" min={10} max={150} value={params.ch} onChange={setParam} />
          <Slider label="Costo mantener ($/hs/mes)" id="cm" min={1} max={80} value={params.cm} onChange={setParam} />
          <Slider label="Capacidad máxima (hs/mes)" id="cap" min={50} max={200} step={5} value={params.cap} onChange={setParam} />
          <div className="divider" />
