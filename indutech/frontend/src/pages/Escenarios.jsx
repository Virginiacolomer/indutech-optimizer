...
const DESCS  = { base: 'Plan estándar sin alteraciones.', prod: 'Caída de productividad del equipo técnico en 20%.', out: 'Tercerización a $8/hs sin costo de activación.', outfix: 'Tercerización a $8/hs + $10.000 de activación.', demand: 'Pico de demanda con aumento del 20%.' };
...
export default function Escenarios() {
  ...
  const chartOpts = { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { grid: { display: false }, ticks: { font: { size: 11 } } }, y: { grid: { color: '#e8e7e4' }, ticks: { font: { size: 10 }, callback: v => '$' + Math.round(v / 1000) + 'k' } } } };

  return (
    <>
      <h2 style={{ fontSize: 15, fontWeight: 600, marginBottom: 4 }}>Comparación de escenarios</h2>
      <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 16 }}>Activá cada escenario para comparar el costo total resultante.</p>
          ...
          <div className="panel">
            ...
            {costs.out && costs.outfix && (
              <div className="alert alert-info" style={{ marginTop: 14, marginBottom: 0 }}>
                ...
                <span>Outsourcing sin costo fijo ahorra {fmt(costs.base - costs.out)} vs. el plan base. Con activación de $10.000 resulta {fmt(costs.outfix)} — más caro que el plan interno.</span>
              </div>
            )}
          </div>
        </div>
